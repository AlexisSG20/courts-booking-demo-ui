...
const DEMO_BOOKINGS = [
  ...
  },
  {
    ...
    totalPrice: 100,
    token: "DEMO-MN34OP56",
    usedAt: "2026-03-31T20:03:00.000Z",
    createdAt: "2026-03-31T16:40:00.000Z",
  },
  {
    id: "demo-5",
    courtId: 2,
...
export default function AdminBookings() {
  ...
  const filteredBookings = useMemo(() => {
    return DEMO_BOOKINGS.filter((booking) => {
      if (courtId && String(booking.courtId) !== String(courtId)) return false;
      if (pending && booking.usedAt) return false;
      if (from && booking.date < from) return false;
      if (to && booking.date > to) return false;
      return true;
    });
  }, [courtId, pending, from, to]);
  ...
  const checkIn = async (token) => {
    const booking = (data.bookings ?? []).find((item) => item.token === token);

    if (!booking) {
      alert("No se encontró la reserva demo.");
      return;
    }

    if (booking.usedAt) {
      alert("Esta reserva demo ya figura como usada.");
      return;
    }

    alert("Ingreso registrado en modo demo.");
  };
