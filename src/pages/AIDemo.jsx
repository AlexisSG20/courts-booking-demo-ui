...
function getDemoAnswer(question) {
  ...
  if (q.includes("cuántas reservas hay") || q.includes("cuantas reservas hay")) {
    return "Actualmente hay 5 reservas demo registradas en el sistema.";
  }

  if (
    q.includes("pendientes") ||
    q.includes("cuántas reservas pendientes") ||
    q.includes("cuantas reservas pendientes")
  ) {
    return "En la demo hay 3 reservas pendientes de ingreso.";
  }

  if (
    q.includes("registraron ingreso") ||
    q.includes("ya fueron usadas") ||
    q.includes("usadas")
  ) {
    return "En la demo hay 2 reservas con ingreso registrado.";
  }

  if (q.includes("usuarios activos")) {
    return "La demo muestra 2 usuarios activos principales: una cuenta ADMIN y una cuenta STAFF.";
  }

  if (q.includes("usuarios hay") || q.includes("usuarios registrados")) {
    return "La demo contempla 2 usuarios operativos: admin@courts.com y staff@courts.com.";
  }

  if (q.includes("reservas recientes") || q.includes("lista las reservas")) {
    return "Reservas demo recientes:\n- Losa 1 | 2026-03-30 | 8:00 - 9:00\n- Losa 2 | 2026-03-31 | 18:00 - 19:00\n- Losa 3 | 2026-03-31 | 20:00 - 21:00";
  }

  if (q.includes("reservas para hoy") || q.includes("hay reservas para hoy")) {
    return "Sí. En la demo hay reservas para hoy en Losa 1 durante la mañana.";
  }

  if (q.includes("qué cancha tiene más reservas") || q.includes("que cancha tiene mas reservas")) {
    return "En la demo, Losa 1 aparece como la cancha con más reservas registradas.";
  }

  if (q.includes("qué es una reserva") || q.includes("que es una reserva")) {
    return "Una reserva representa la asignación de una cancha en una fecha y rango horario, con cantidad de personas, token y estado de ingreso.";
  }

  if (q.includes("qué puedes responder") || q.includes("que puedes responder")) {
    return "Puedo responder preguntas demo sobre reservas, ingresos, usuarios y canchas. Esta versión pública no consulta la base real.";
  }

  return "Esta es una respuesta demo. Prueba con preguntas sobre reservas, ingresos, usuarios o canchas.";
}
      ...
      <section className="relative mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
        <section className="relative isolate grid gap-6 overflow-hidden rounded-[2rem] border border-white/16 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.24)] lg:grid-cols-[1.5fr_0.9fr] lg:p-10">
          ...
          <div className="relative space-y-6">
            ...
            <div className="space-y-4">
              ...
              <h1 className="max-w-4xl text-4xl font-semibold tracking-tight text-white sm:text-5xl">
                Explora una interfaz tipo chat con respuestas demo.
              </h1>

              <p className="max-w-3xl text-base leading-8 text-slate-200/82 sm:text-lg">
                Esta versión pública simula preguntas operativas sobre reservas,
                usuarios y canchas para mostrar el flujo visual del proyecto.
              </p>
            </div>
          </div>
          ...
          <div className="relative grid gap-4">
            ...
            <article className="rounded-[24px] border border-white/12 bg-slate-950/34 p-5 backdrop-blur-sm">
              <p className="text-sm text-sky-200">Origen</p>
              <p className="mt-3 text-2xl font-semibold text-white">
          ...
          <aside className="rounded-[2rem] border border-white/12 bg-slate-950/44 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.2)] backdrop-blur-md">
            ...
            <article className="mt-6 rounded-[24px] border border-emerald-400/24 bg-emerald-400/10 p-4">
              ...
              <p className="mt-3 text-sm leading-7 text-emerald-50/90">
                Esta demo responde preguntas visuales sobre reservas, usuarios y
                canchas. La versión completa real se muestra en el video demo y en
